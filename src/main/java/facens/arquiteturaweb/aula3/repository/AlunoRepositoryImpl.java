...
public class AlunoRepositoryImpl implements AlunoRepository {
    ...
    public List<Aluno> findAll() {
        ...
        return jdbcTemplate.query("SELECT * FROM aluno", (resultSet, rowNum) -> {
            ...
            return new Aluno(
                    ...
                    resultSet.getString("tipoensino"),
    ...
    public Aluno save(facens.arquiteturaweb.aula3.modelo.Aluno aluno) {
        if (aluno.getId() != null) {
            String insertQuery = "INSERT INTO public.aluno (id, nome, curso, ra, tipoensino, genero) VALUES (?, ?, ?, ?, ?, ?)";
            /*
             ...
             */
            jdbcTemplate.update(insertQuery, aluno.getId(), aluno.getNome(), aluno.getCurso(), aluno.getRa(),
                    aluno.getTipoEnsino(), aluno.getGenero());
        } else {
            String updateQuery = "UPDATE public.aluno SET nome = ?, curso = ?, ra = ?, tipoensino = ?, genero = ? WHERE id = ?";
            jdbcTemplate.update(updateQuery, aluno.getNome(), aluno.getCurso(), aluno.getRa(), aluno.getTipoEnsino(),
                    aluno.getGenero(), aluno.getId());
        }
        return aluno;
    }
}
